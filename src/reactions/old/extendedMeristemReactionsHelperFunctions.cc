...
//if L1nodimer is activated. No dimerization in L1
        ...
        else if (neighborOnlyCoopt==2)//Betul style cooperativity: Monomer
        {
            ...
            if (m4Flag==1) //AD ADDITION 082521
                neighbors += 2;
    ...
    else if (crmOccupancy[i]==2)
    {
        ...
        else if (neighborOnlyCoopt==2)//Betul style cooperativity: Dimer
        {
            ...
            if (m4Flag==1 && i==1) //m4 gets cooperativity from all other sites.
            {
                ...
                if (neighbors > 3) //AD Addition: Trying to bring down m4 repression 082021
                    neighbors = 3;
                   ...
                   int &crmSite, Compartment &compartment, int chrom, int monFireLimit, int crmOrMarkerSwitch, int polTimeLimit, int unbindLimit)
{
    ...
    int monKeepAgeRoll=0;
    int neighbors = 0;
    int nearNeighbor1 = 0;
    int nearNeighbor2 = 0;
    int neighborsInInterval = 0;

    if (eventBegin <= randvalue2 && eventEnd >= randvalue2)//MOD: 052321 random values exactly at the border are probably more ideally handled by randomly assigning them to either the left or right event but >= seems and acceptable compromise.
    {
...
//timer only for monomers with no cooperativity
         ...
         else if (crmSite==2)
         {
             ...
             crmSiteActiveTimer=0;//MOD 052321 changed == to =
...
double TimeStepGenerator(int crmOrMarkerSwitch, double randvalue1, int polTimeLimit, Compartment &compartment)
{
   ...
   double timeStepStochastic=0;
    ...
    timers[2]=compartment.rnaPolTimer[crmOrMarkerSwitch][1];//Examine later MOD 052321 [0] to [1]

    for (tgI = 3; tgI <= 7; tgI++)
    timers[tgI]=compartment.unbindTimer[timerSet[0]][tgI-3];//gather the monomer timers across both crms
